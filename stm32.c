...
static stm32_err_t stm32_send_init_seq(const stm32_t *stm)
{
	...
	//sleep(100);
	fprintf(stderr, "11write %d\n", p_err);
	if (p_err != PORT_ERR_OK) {
		fprintf(stderr, "Failed to send init to device\n");
		fprintf(stderr, "back 11\n");
		return STM32_ERR_UNKNOWN;
		}
	p_err = port->read(port, &byte, 1);

	fprintf(stderr, "11read %d\n", p_err);
	if (p_err == PORT_ERR_OK && byte == STM32_ACK){
		fprintf(stderr, "back 12\n");
		return STM32_ERR_OK;
		}
	if (1)
		return STM32_ERR_OK;	
	...
	if (p_err == PORT_ERR_OK && byte == STM32_NACK) {
		...
		fprintf(stderr, "back 13\n");
		return STM32_ERR_OK;
	}
	if (p_err != PORT_ERR_TIMEDOUT) {
		fprintf(stderr, "Failed to init device.\n");
		fprintf(stderr, "back 14\n");
		return STM32_ERR_UNKNOWN;
	}
	
	fprintf(stderr, "111end %d\n", p_err);
	/*
	 * Check if previous STM32_CMD_INIT was taken as first byte
	 * of a command. Send a new byte, we should get back a NACK.
	 */
	p_err = port->write(port, &cmd, 1);
	fprintf(stderr, "22write %d\n", p_err);
	if (p_err != PORT_ERR_OK) {
		fprintf(stderr, "Failed to send init to device\n");
		fprintf(stderr, "back 21\n");
		return STM32_ERR_UNKNOWN;
		}
	p_err = port->read(port, &byte, 1);
	fprintf(stderr, "22read %d\n", p_err);
	if (p_err == PORT_ERR_OK && byte == STM32_NACK){
		fprintf(stderr, "back 22\n");
		return STM32_ERR_OK;
		}
	fprintf(stderr, "Failed to init device.\n");
	fprintf(stderr, "22end\n");
	return STM32_ERR_UNKNOWN;
}
